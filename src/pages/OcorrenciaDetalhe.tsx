...
export const OcorrenciaDetalhe: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const proposta = ativas[0];
    if (STATUS_PROTEGIDOS.includes(proposta.status)) return;

    (async () => {
      await DataStore.saveOrcamento({ ...proposta, status: statusEsperado as OrcamentoStatus });
      await loadData();
    })();
  ...
  return (
    <div
      ...
      className="flex flex-col h-full min-h-0 w-full bg-[#0D1117] font-body overflow-y-auto"
    >
      ...
      <div className="flex-1 px-3 md:px-4 py-3 pb-6">
